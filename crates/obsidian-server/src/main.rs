...
#[cfg(debug_assertions)]
fn configure_static(cfg: &mut web::ServiceConfig) {
    // Serve from Vite build output. In dev, run `npm run build:watch` inside frontend/
    // or run `npm run dev` to use the Vite dev server (port 5173) with HMR.
...
async fn main() -> std::io::Result<()> {
    ...
    if config.auth.jwt_secret.trim().is_empty() {
        ...
        warn!(
            "auth.jwt_secret is empty; generated an ephemeral runtime secret. Set [auth].jwt_secret in config.toml for persistent tokens across restarts."
        );
    } else if config.auth.jwt_secret.trim() == obsidian_host::config::DEFAULT_DEV_JWT_SECRET {
        warn!(
            "Using insecure default JWT secret. Set [auth].jwt_secret in config.toml before production use."
        );
    }
        ...
        .await
    {
        ...
        Err(e) => {
            warn!(
                "User bootstrap skipped: {}. Configure [auth] bootstrap_admin_username/bootstrap_admin_password in config.toml to create the first admin.",
                e
            );
        }
    }
    ...
    HttpServer::new(move || {
        ...
        App::new()
            ...
            .configure(obsidian_host::routes::version::configure)
